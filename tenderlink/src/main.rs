fn main() {
    ...
    let peers = vec![tenderlink::bandwidth_test::STPAddress { ip: "::1".parse().unwrap(), port: P2P_PORT, magic1: tenderlink::bandwidth_test::CONNECT_MAGIC1_PLAIN_TEXT, key: Vec::from(&[0xAAu8; 32]) }];
    if args.len() == 1 {
